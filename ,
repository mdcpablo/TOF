...
def time_of_flight(mu, I, N, q, sigt_Fe, sigt_U, v, de, tau, option):
    ...
    # Inputs:
    #   mu .......... direction
    #   I ........... number of spatial cells for source
    #   N ........... number of time steps
    #   q[g] ........ source term (n/cm^2-s) for group g @ left boundary (x=0cm)  
    #   sigt_Fe[g] .. iron pure-absorber cross section (1/cm) for group g
    #   sigt_U[g] ... U235 pure-absorber cross section (1/cm) for group g
    ...
    #   num_neutrons[n] ... number of neutrons @ detector (x=100cm) for time bin n
    # -----------------------------------------------------------------------------

    if len(sigt_U) != len(sigt_Fe): print "Fatal Error: len(sigt_U) != len(sigt_Fe)"; return

    (t, dt) = time_spacing(N, option)
    ...
    for i in range(I):
        ...
        for g in range(G): 
            if g % (G/20) == 0: print "  "+str(int(float(g)/float(G)*100.))+"%"
            # time at which pulse first reaches the detector (x=100cm)
            pulse_start_time = (100.-x_source[i])/(mu*v[g]) 
            # time at which neutrons are no longer reaching the detector (x=100cm)
            pulse_end_time = (100.-x_source[i])/(mu*v[g]) + tau
    
            # neutrons of velocity v[g] contribute to a small contiguous segment of the "num_neutrons" array ...
            # finding the starting and ending indices of "num_neutrons" array that neutrons of velocity v[g] contribute to
            ...
            for n in range(N_start, N_end):
                ...
                num_neutrons[n] += delta_t*dx[i]*de[g]*q[g]*np.exp(-sigt_U[g]*(0.05-x_source[i])/mu)*np.exp(-2.06*sigt_Fe[g]/mu) # (n/cm^2)
...
def time_of_flight_subelements(mu, I, N, q, sigt_Fe, sigt_U, v, de_sub, tau, option):
    ...
    #   sigt_Fe[g] .. iron pure-absorber cross section (1/cm) for group g
    #   sigt_U[g] ... U235 pure-absorber cross section (1/cm) for group g
    #   v[s] ........ speed (cm/s) for subelement s
    #   de_sub[s] ... energy width for subelement s
    #   tau ......... lifespan of pulse (s), pulse lasts from t=0 to t=tau
    #   option ...... 'lin' (linear) or 'log' (logarithmic) time bins for time-of-flight calculation 
    #
    # Outputs:
    ...
    for i in range(I):
        ...
        for s in range(S): 
            g = int(de_sub[s,0])
            if s % (S/20) == 0: print "  "+str(int(float(s)/float(S)*100.))+"%"
            ...
            for n in range(N_start, N_end):
                delta_t = max(0, min(pulse_end_time, t[n]+0.5*dt[n]) - max(pulse_start_time, t[n]-0.5*dt[n]) )
                num_neutrons[n] += delta_t*dx[i]*de_sub[s,1]*q[s]*np.exp(-sigt_U[g]*(0.05-x_source[i])/mu)*np.exp(-2.06*sigt_Fe[g]/mu) # (n/cm^2)
...
def make_xs_plots():
    ...
    '''
###############################################################################
def run_with_coarse_time_step(xsdir, exact):
    # energy at midpoint of energy group (MeV)
    emid = np.loadtxt(os.path.join(xsdir,'emid'), skiprows=1) 
    # width of energy group (MeV)
    de = np.loadtxt(os.path.join(xsdir,'de'), skiprows=1) 
    ...
    (t_fine, signal_fine) = time_of_flight(mu, I, N_fine, q, sigt_Fe, sigt_U, v, de, tau, option)
    #error = np.linalg.norm(exact-signal)/np.linalg.norm(exact)
  
    if option == 'log':
        detector_start_time = 1e-10
        detector_end_time = 4e-7
        times = np.logspace(np.log10(detector_start_time), np.log10(detector_end_time), num=N_fine+1)
        dt_fine = times[1:N_fine+1] - times[0:N_fine]
        t_fine = 0.5*(times[1:N_fine+1] + times[0:N_fine])
        times = np.logspace(np.log10(detector_start_time), np.log10(detector_end_time), num=N_coarse+1)
        dt_coarse = times[1:N_coarse+1] - times[0:N_coarse]
        t_coarse = 0.5*(times[1:N_coarse+1] + times[0:N_coarse])
    elif option == 'lin':
        detector_start_time = 0
        detector_end_time = 4e-7
        times = np.linspace(detector_start_time, detector_end_time, num=N_fine+1)
        dt_fine = times[1:N_fine+1] - times[0:N_fine]
        t_fine = 0.5*(times[1:N_fine+1] + times[0:N_fine])
        times = np.linspace(detector_start_time, detector_end_time, num=N_coarse+1)
        dt_coarse = times[1:N_coarse+1] - times[0:N_coarse]
        t_coarse = 0.5*(times[1:N_coarse+1] + times[0:N_coarse])
    else:
        print " ** Fatal Error: option must be 'lin' or 'log'"     

    signal_coarse = np.zeros(N_fine)

    for n_coarse in range(N_coarse):
        for n_inner_loop in range(N_fine/N_coarse):
            n_fine = n_coarse*(N_fine/N_coarse) + n_inner_loop
            signal_coarse[n_coarse*(N_fine/N_coarse):(n_coarse+1)*(N_fine/N_coarse)] += signal_fine[n_fine]

    plt.semilogy(t_fine,signal_coarse+1e-21*np.ones(len(signal_coarse)))
    plt.xlim([0, 3.3e-7])
    plt.ylim([1e-18, 1e-12])
    #plt.show()
    error = np.linalg.norm(exact-signal_coarse)/np.linalg.norm(exact)
    print error
    #return signal_coarse, error
###############################################################################
def run_subelements_with_coarse_time_step(xsdir, exact):
    # energy at midpoint of energy group (MeV)
    emid = np.loadtxt(os.path.join(xsdir,'emid'), skiprows=1) 
    # width of energy group (MeV)
    de = np.loadtxt(os.path.join(xsdir,'de'), skiprows=1) 
    ...
    (t_fine,signal_fine) = time_of_flight_subelements(mu, I, N_fine, q, sigt_Fe, sigt_U, v, de_sub, tau, option) 
